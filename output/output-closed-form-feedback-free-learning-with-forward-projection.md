...
\widetilde Z_l
=
\underbrace{g_l(A_{l-1}Q_l)}_{\text{retain input information}}
+
\underbrace{g_l(YU_l)}_{\text{inject label information}}
\]
...
Once the target \(\widetilde Z_l\) has been created, Forward Projection chooses \(W_l\) so that:

\[
A_{l-1}W_l \approx \widetilde Z_l
\]
...
W_l =
\left(
A_{l-1}^{\top}A_{l-1}+\lambda I
\right)^{-1}
A_{l-1}^{\top}\widetilde Z_l
\]
...
The regularization parameter \(\lambda\) discourages huge weights and improves the conditioning of the matrix solve.
